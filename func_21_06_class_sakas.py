...
class AEC_model(object):
    ...
    def io_func(self,key):
        ...
        if key=='Kibria anode-high':
            E=71
            #E=65.34450651858272 #correction
        else:
            E=41
            #E=35.36669440152163
            #E=DF_ARR.loc[key][2] 
   ...
   #%% reversible voltage calculation     
    ...
    def anodic_over_Kibria_high(self,J):#J expressed in [mA/cm^2] 
        ...
        # i_o=np.polyval(io_par[key],self.T)+(4.39*(0.3-self.w)) #polynomial
        i_o=self.io_func(key)
        #A=np.polyval(tafel_par[key],self.T)+(0.32*((0.3-self.w))) 
        # i_o=self.io_func(key)+(4.629*(0.3-self.w)) #wt correction
        A=np.polyval(tafel_par[key],self.T)
        eta_an=A*np.log10(J/i_o)
        return eta_an,i_o
    ...
    def cathodic_over_Kibria_high(self,J):#J expressed in [mA/cm^2]   
        ...
        i_o=self.io_func(key)
        A=np.polyval(tafel_par[key],self.T)
        # i_o=np.polyval(io_par[key],self.T)+(39*(0.3-self.w)) #Polynomial fit
        # #i_o=self.io_func(key)
        # A=np.polyval(tafel_par[key],self.T)+(0.355*(0.3-self.w)) 
        eta_an=A*np.log10(J/i_o)
        return eta_an,i_o
    ...
    #     return J*0.00001-4*10**(-9)*(J)**2+10**(-9)*(J)**3 #1000 is used to convet mA/cm^2 into A/cm^2
    ...
    def eta_ohm_bub(self,J): 
        ...
        sigma_l_f=self.sigma_liquid()
        sigma_b_f=sigma_l_f/((1/(1-self.theta_eps(J)[1])**(1.5))-1) #Bruggman relationship [S/cm]
        return J*self.delta_l/(sigma_b_f*1000) #1000 is used to convet mA/cm^2 into A/cm^2
...
#%% only omic 
    def kibria_bub_ohm(self,x):
        """
        Polarisation curve with Kibria conditions and high overvoltage working region; 1 sets of i0 and alphas
        Only ohmic bubble effects are added 
        also the elctrode over potential is added
    ...
    #%% single current density
    def kibria_mono(self,x):
        """
        Polarisation curve with Kibria conditions without bubble effects
    ...
    def kibria_fsolve(self,J,V_cell):
        """
        Construct the function of fsolve, which is the polarisation curve described as Vcell =f(J)
        ...
        i_o=self.io_func(key)
        A=np.polyval(tafel_par[key],self.T)       
        eta_ca=A*np.log10(J/i_o)
        
        R=np.polyval(VE_par["poly"],self.T)[0]  #[ohm * cm^2]
        eta_d=R*J/1000 #mA/cm^2 into A/cm^2
        
        eta_l=(self.delta_l*J/(self.sigma_liquid()*1000)) 
        
        
        
        eq=self.v_rev()+eta_an+eta_ca+eta_d+eta_l-V_cell
        return eq
     
    def J_fsolve_kibria(self,V_cell):
        """
        Actula fsolve process to get J=f(Vcell)
        ...
        """
        root = fsolve(self.kibria_fsolve, [(V_cell-1.23)/(0.4*1000)],(V_cell)) # the initial condition is using ohmic law with ASR=0.4
        #root = fsolve(self.kibria_fsolve, [2],(V_cell))
...
#%%
    def E_fsolve_func(self,E,io):
        ...
        return io_ref*np.exp((E/R)*((1/T_ref)-(1/self.T)))-io
    ...
    def E_fsolve(self,io):
        
        root = fsolve(self.E_fsolve_func, [20],(io)) # the initial condition is using ohmic law with ASR=0.4
        #root = fsolve(self.kibria_fsolve, [2],(V_cell))
